    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            output_states = layer_module(
                next_kv.float(), # if i >= self.range_of_fixed_layer else next_kv.half(),
                attention_mask,
                output_attentions,
                query_states=query_states,
                relative_pos=relative_pos,
                rel_embeddings=rel_embeddings.float(), # if i >= self.range_of_fixed_layer else rel_embeddings.half(),
            ).float()
...
class DebertaV2Embeddings(nn.Module):
    ...
    def forward(self, input_ids=None, token_type_ids=None, position_ids=None, mask=None, inputs_embeds=None):
        ...
        return embeddings, position_embeddings#.repeat(embeddings.shape[0], 1, 1)
...
class DebertaV2Model(DebertaV2PreTrainedModel):
    ...
    def forward(
        ...
        embedding_output, positional_embeddings = self.embeddings(
            input_ids=input_ids,
...
class DebertaV2ForMaskedLM(DebertaV2PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.lm_predictions = EnhancedMaskDecoder(config, 128100)
    ...
    def fix_layers(self, num_of_layer=None):
        ...
        for param in self.get_input_embeddings().parameters():
            param.requires_grad = False
        for param in self.lm_predictions.lm_head.parameters():
            param.requires_grad = False


    def get_output_embeddings(self):
        return self.lm_predictions.lm_head.dense.weight

    def set_output_embeddings(self, new_embeddings):
        self.lm_predictions.lm_head.dense.weight = new_embeddings
    ...
    def forward(
        ...
        **kwargs
        ...
        outputs, positional_embeddings = self.deberta(
            input_ids,
        ...
        z_states = positional_embeddings
        lm_labels = labels
...
class EnhancedMaskDecoder(torch.nn.Module):
    ...
    def forward(self, ctx_layers, ebd_weight, target_ids, input_ids, input_mask, z_states, attention_mask, encoder,
                relative_pos=None, *wargs, **kwargs):
        ...
        lm_logits = self.lm_head(ctx_layer, ebd_weight).float()  # bz * max_seq_length * vocab_size
        lm_labels = target_ids
        lm_loss = None
        if target_ids is not None:
            _lm_logits = lm_logits.view(-1, lm_logits.shape[-1])  # (bz * max_seq_length) * vocab_size
            _lm_labels = target_ids.view(-1)
            label_index = (target_ids.view(-1) > 0).nonzero().view(-1)
            _lm_labels = _lm_labels.index_select(0, label_index)
            _lm_logits = _lm_logits.index_select(0, label_index)
            lm_loss = loss_fct(_lm_logits, _lm_labels.long()).sum()
